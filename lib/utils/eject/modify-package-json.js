...
const ensureLatestSkyuxPackages = require('./ensure-latest-skyux-packages');
...
async function modifyPackageJson(ejectedProjectPath) {
  ...
  logger.info('Checking for outdated SKY UX packages...');
  await ensureLatestSkyuxPackages(dependencies);
  await ensureLatestSkyuxPackages(devDependencies);
