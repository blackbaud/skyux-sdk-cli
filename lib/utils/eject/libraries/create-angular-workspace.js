...
function createAngularWorkspace(ejectedProjectPath, workspaceName, strictMode) {
  const ejectedProjectDir = path.basename(ejectedProjectPath);

  logger.info(
    `Creating an empty Angular workspace named "${workspaceName}", located at "./${ejectedProjectDir}"...`
  );
