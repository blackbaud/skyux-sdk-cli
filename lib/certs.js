...
async function trustLinux() {
  await execute('trust', 'OS', async () => {
    await spawn(`sudo`, `cp`, certUtils.getCertAuthPath(), `/usr/local/share/ca-certificates/${certUtils.getCertAuthName()}`);
    await spawn(`sudo`, `update-ca-certificates`);
  });

  if (certUtils.getLinuxChromeNSSPathExists()) {
    await execute('trust', 'NSS Chrome', async () => {
      await spawn(`certutil`, `-d`, `sql:${certUtils.getLinuxChromeNSSPath()}`, `-A`, `-t`, `P`, `-n`, certUtils.getCertAuthCommonName(), `-i`, certUtils.getCertPath());
    });
  } else {
...
async function untrustLinux() {
  await execute('untrust', 'OS', async () => {
    await spawn(`sudo`, `rm`, `-rf`, `/usr/local/share/ca-certificates/${certUtils.getCertAuthName()}`);
    await spawn(`sudo`, `update-ca-certificates`, `--fresh`);
  });

  if (certUtils.getLinuxChromeNSSPathExists()) {
    await execute('untrust', 'NSS Chrome', async () => {
      await spawn(`certutil`, `-D`, `-d`, `sql:${certUtils.getLinuxChromeNSSPath()}`,`-n`, certUtils.getCertAuthCommonName());
    });
  } else {
...
async function trustMac() {
  await execute('trust', 'OS', async () => {
    await spawn(`sudo`, `security`, `add-trusted-cert`, `-d`, `-r`, `trustRoot`, `-k`, `/Library/Keychains/System.keychain`, certUtils.getCertAuthPath());
  });
}

async function untrustMac() {

  // Backwards compatability when we trusted the cert instead of the CA.
  try {
    await execute('untrust', 'OS', async () => {
      await spawn(`sudo`, `security`, `delete-certificate`, `-c`, certUtils.getCertCommonName());
    });
  } catch (err) { 
    // Ignoring any error since the trusted cert will eventually not exist
  }

  await execute('untrust', 'OS', async () => {
    await spawn(`sudo`, `security`, `delete-certificate`, `-c`, certUtils.getCertAuthCommonName());
  });
}

async function trustWindows(argv) {
  const cmdArgs = ['/c', 'certutil', '-addstore', '-f', 'root', `""${certUtils.getCertAuthPath()}""`];
...
async function untrustWindows(argv) {
  const cmdArgs = ['/c', 'certutil', '-delstore', 'root', certUtils.getCertAuthCommonName()];
