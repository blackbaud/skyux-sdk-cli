...
async function eject() {
  try {
    ...
    const enableStrictMode = await promptForStrictMode(CWD);

    const libraryPath = path.join(process.cwd(), 'src/app/public');
    const isLibrary = fs.existsSync(libraryPath);
    if (isLibrary) {
      return ejectLibrary(ejectedProjectPath, isInternal, enableStrictMode);
    }
    ...
    backupSourceFiles(ejectedProjectPath, SOURCE_CODE_BACKUP_DIR);

    // Setup project and packages.
    createAngularCliProject(ejectedProjectPath, projectName, enableStrictMode);
