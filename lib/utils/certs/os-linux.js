...
function logAutomationSkip(action, level) {
  logger.info(
    `Skipped ${action}ing the SKY UX certificate at the ${level} level.`
  );
}
...
async function trust() {
  await execute('trust', 'OS', async () => {
    await spawn(
      ...
      `/usr/local/share/ca-certificates/${generator.getCertAuthName()}`
    );
...
async function untrust() {
  await execute('untrust', 'OS', async () => {
    await spawn(
      ...
      `rm`,
      `-rf`,
      `/usr/local/share/ca-certificates/${generator.getCertName()}`
    );
    await spawn(
      `sudo`,
      `rm`,
