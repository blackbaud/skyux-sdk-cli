...
async function eject() {
  try {
    ...
    const publicDirectory = path.join(process.cwd(), 'src/app/public');
    const hasLibrary = fs.existsSync(publicDirectory);
    let spaName = projectName;
    let libraryName = projectName;
    if (hasLibrary) {
      spaName += '-spa';
    }

    // Setup project and packages.
    createAngularCliProject(ejectedProjectPath, spaName);
    migrateSkyuxConfigAppStylesArray(skyuxConfig, ejectedProjectPath, spaName);
    ...
    if (hasLibrary) {
      ...
      migrateLibraries.copyFiles(publicDirectory, ejectedProjectPath, libraryName);
      migrateLibraries.modifyPackageJson(ejectedProjectPath);
    }
