...
const moveEjectedFilesToRoot = require('./utils/eject/move-ejected-files');
const moveSourceFilesToTemp = require('./utils/eject/move-source-files');
...
const SOURCE_CODE_BACKUP_TEMP_DIR = '.skyux4sourcecode';
const EJECTED_FILES_TEMP_DIR = '.skyuxejecttemp';
...
async function eject() {
  try {
    ...
    const ejectedProjectPath = path.join(CWD, EJECTED_FILES_TEMP_DIR);
    ...
    // Move files.
    moveSourceFilesToTemp(ejectedProjectPath, SOURCE_CODE_BACKUP_TEMP_DIR);
    moveEjectedFilesToRoot(ejectedProjectPath);

    // Run `npm install` after all files have been merged to regen the node_modules.
    // (Installing the modules again is faster than copying node_modules directly.)
    await npmInstall();
