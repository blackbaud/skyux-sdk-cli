...
async function addSkyPeerDependencies(dependencies) {
  ...
  if (dependencies) {
    ...
    const skyPackageNames = Object.keys(validDependencies).filter(
      (packageName) => {
        return (
    ...
    for (const packageJson of packageJsonList) {
      ...
      if (packageJson.peerDependencies) {
        for (const packageName of Object.keys(packageJson.peerDependencies)) {
          ...
          logger.info(
            `Added package ${packageName}@${packageJson.peerDependencies[packageName]} since it is a peer dependency of ${packageJson.name}@${packageJson.version}.`
          );
    ...
    if (ignoredPackages.length) {
      logger.info(
        'Warning: The following missing peer dependencies were not added because they are not known Angular or SKY UX packages. Do you need to install them yourself?'
      );
      logger.info(LOG_SEPARATOR);
      ignoredPackages.forEach((p) =>
        logger.info(`${p.missingPeer} --> peer of ${p.peer}`)
      );
      logger.info(LOG_SEPARATOR);
    }
...
async function upgradeDependencies(dependencies) {
  if (dependencies) {
    ...
    const { validDependencies, invalidDependencies } =
    ...
    const packageConfigs = [
      ...
      {
        regexp: /^@angular-devkit\//,
        version: '^12.0.0'
      },
      {
        regexp:
          /^@blackbaud\/skyux-lib-(clipboard|code-block|media|restricted-view|stache)$/,
        version: '^4.0.0-alpha.0 || ^5.0.0-alpha.0'
      },
      {
      ...
      {
        regexp: /^@skyux-sdk\/(e2e|pact|testing)$/,
        version: '^4.0.0-alpha.0 || ^5.0.0-alpha.0'
      },
      {
      ...
      Object.keys(validDependencies).forEach((packageName) => {
        if (
          ...
          if (packageConfig.reason) {
            logger.info(
              `Note: Using version range ${packageName}@${packageConfig.version} because ${packageConfig.reason}.`
            );
          }
        }
      });
    ...
    for (const packageName in dependencyRanges) {
      ...
      logger.info(
        `Finding latest version within range ${packageName}@${versionRange}...`
      );
      promises.push(
        latestVersion(packageName, {
          version: `${versionRange}`
        }).catch(() => {
          // If a valid version is not found, revert to what's in package.json.
          invalidDependencies[packageName] = versionRange;
          return validDependencies[packageName];
        })
      );
    }
    ...
    Object.keys(validDependencies).forEach((packageName, index) => {
      ...
      if (validDependencies[packageName] === versionNumber) {
        ignoreMessages.push(
          `Skipped package ${packageName} because it is already set to (${versionNumber}).`
        );
      } else {
        updateMessages.push(
          `Updated package ${packageName} to (${versionNumber}).`
        );
      }
    });
    ...
    if (invalidPackages.length) {
      invalidPackages.forEach((packageName) => {
        ...
        logger.warn(
          `Warning: Skipped package ${packageName} because it did not provide a valid version or range: "${version}".`
        );
      });
      logger.info(LOG_SEPARATOR);
      logger.info(
        `Total packages with invalid versions: ${invalidPackages.length}`
      );
    }
