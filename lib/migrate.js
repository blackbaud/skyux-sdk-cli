...
function validateDependencies(packageJson) {
  ...
  const packages = [
    ...
    },
    {
      ...
      reason:
        "it has been deprecated in favor of @skyux-sdk/testing. Test fixtures have been migrated to their respective library's testing module"
    },
          ...
          for (let packageName in packageJson[section]) {
            if (packageToRemove.name.test(packageName)) {
              ...
              if (packageToRemove.reason) {
                logger.info(
                  `Removed ${packageName} from ${section} because ${packageToRemove.reason}.`
                );
              }
            }
          }
