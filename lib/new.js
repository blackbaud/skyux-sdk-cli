...
async function updatePackageDependencies() {
  ...
  const descriptionType = getPackageDescriptionType(settings);
...
function promptForName(args) {
  const validator = (value) => {
    ...
    const packageDescriptionType = getPackageDescriptionType(settings);
    ...
    logger.info(`Creating a new ${packageDescriptionType} named '${packageName}'.`);
...
async function skyuxNew(argv) {
  try {
    ...
    logger.info(`${getPackageDescriptionType(settings)} created in directory ${settings.name}`);
