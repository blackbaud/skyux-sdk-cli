...
function cleanupTemplate() {
  try {
    ...
    fs.copySync(settings.pathTmp, settings.path, {
      mkdirp: true,
      clobber: true
    });
...
async function setDependencyVersions(dependencies) {
  ...
  const dependencyPromises = packageNames.map((packageName) =>
    latestVersion(packageName, { version: dependencies[packageName] })
  );
...
async function updatePackageDependencies() {
  ...
  packageJson.description = `A ${getPackageDescriptionType(settings)} named ${
    settings.name
  }`;
...
async function cloneTemplate(argv) {
  const message = logger.promise(
    `Cloning ${settings.template.name} SKY UX template.\n`
  );
  ...
  try {
    ...
    if (err.message.indexOf('status 1,') > -1) {
      message.fail(
        `Template found but missing corresponding ${err.branch} branch. Please consult the template owner or use the '--branch' flag.`
      );
  ...
  const prompt = () => {
    ...
    return inquirer
      ...
      .then((answer) => {
        ...
        logger.info(
          `\nCreating a ${getPackageDescriptionType(
            settings
          )} named '${getPackageName(value)}'...`
        );
        return value;
      });
  };
...
async function skyuxNew(argv) {
  try {
    ...
    logger.info(
      `Created a ${getPackageDescriptionType(settings)} in directory ${
        settings.name
      }`
    );
    logger.info(
