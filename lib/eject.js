...
const moveEjectedFiles = require('./utils/eject/move-ejected-files');
...
function moveSourceFiles() {
  ...
  const tempDir = '.skyux4sourcecode';
  ...
  files.forEach(file => {
    fs.moveSync(
      file,
      path.join(CWD, tempDir, file.replace(CWD, ''))
    );
  });
...
async function eject() {
  try {
    ...
    // Move the ejected files in the temp folder to the project's root directory.
    moveEjectedFiles(ejectedProjectPath);
