...
const runLintFix = require('./utils/eject/run-lint-fix');
const setupDeprecatedTsLintBuilder = require('./utils/eject/setup-deprecated-tslint-builder');
...
async function eject() {
  try {
    ...
    // Add the SKY UX Angular builder.
    const builderPackageName = installAngularBuilders(ejectedProjectPath, isInternal);

    // Setup the deprecated TSLint builder.
    setupDeprecatedTsLintBuilder(ejectedProjectPath, projectName, builderPackageName);

    // Update SKY UX dependencies.
    await modifyPackageJson(ejectedProjectPath);

    // Move ejected files.
    moveEjectedFilesToCwd(ejectedProjectPath, projectName);
